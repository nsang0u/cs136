...
1. The if statements disappear because they exist to check conditions of the next and previous arguments. If they are null, the method will account for that in some way.

2. The indexOf method cannot be written using the contains method because contains only returns a boolean and thus would not be particularly helpful. 

3. There is no choice with remove because it specifies one element and impacts both surrounding nodes. Thus there is no concept similar to insertBefore and insertAfter. We don't pass dummy nodes to remove; there are always nodes on either side of the node we are removing. 

4. If the index is 0, just use add first. If the index is the last node, use addLast. This is desirable because it skips the creation of the temp counter variable 'hand,' the while loop and the other operations in the add code that would be unnecessary in these cases. 

5. In my case, the final result source is bigger because the old code is still present, only commented out. Were I to have deleted it all, the final result source would have been smaller.  
 */
...
public class LinkedList<E> extends DoublyLinkedList<E>
{
	...
	DoublyLinkedNode<E> dummyNode1 = new DoublyLinkedNode<E>(null);
	DoublyLinkedNode<E> dummyNode2 = new DoublyLinkedNode<E>(null);
	...
	*/
	...
	//insert value after previous (value, previous) (this is for insertAfter)

	Assert.pre((0 <= i) &&
		   (i <= size()), "Index in range.");
	int hand = 0;
	DoublyLinkedNode<E> holding = head.next();
	while (hand < i){
	    holding = holding.next();
	    hand++;
	}
	// Holding is now node in front of desired index. 
	insertAfter(o, holding);
	//---
	...
	*/
	...
	// Holding is now node to be removed.
	...
	*/
	...
	// Holding is now node to be returned.
	...
	*/
	...
	int hand = (size() - 1);
	DoublyLinkedNode<E> holding = tail.previous();
	while (hand >= 0){
	    if (holding.value() == value){
		return hand;
	    }
	    holding = holding.previous();
	    hand--;
	}
	return -1;
	/*int i = size()-1;
	...
	*/
	...
	int removeVal = indexOf(value);
	return remove(removeVal);
	/*DoublyLinkedNode<E> finger = head;
	while (finger != null &&
